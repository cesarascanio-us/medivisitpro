...
-- =====================================================
-- MASTER RECURSION FIX (v2) - MediVisitPro
-- Date: 2026-01-02
-- Purpose: Break infinite loops in RLS by recreatng the plain cache table.
-- =====================================================

-- 1. RECREATE PLAIN TABLE (Ensures schema match)
...
-- 2. DYNAMIC SYNC (Handles potential missing source columns during migration)
DO $$ 
BEGIN
...
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Initial sync had issues, but schema is now correct for future updates.';
END $$;
...
-- 4. NON-RECURSIVE HELPER FUNCTIONS
...
RETURNS BOOLEAN AS $$
    SELECT EXISTS (
        ...
        AND (supervisor_id = auth.uid() OR supervisor_id IN (SELECT user_id FROM public.user_roles_plain WHERE supervisor_id = auth.uid()))
    );
$$ LANGUAGE sql STABLE SECURITY DEFINER;

-- 5. RE-APPLY POLICIES (BREAKING THE RECURSION)
