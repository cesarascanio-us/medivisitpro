...
DO $$ BEGIN DROP POLICY IF EXISTS "RBAC Transfer Orders Select" ON transfer_orders;
EXCEPTION
WHEN undefined_table THEN NULL;
END $$;
DO $$ BEGIN DROP POLICY IF EXISTS "Org Transfer Orders Access" ON transfer_orders;
EXCEPTION
WHEN undefined_table THEN NULL;
END $$;
DO $$ BEGIN DROP POLICY IF EXISTS "RBAC Expenses Select" ON expenses;
EXCEPTION
WHEN undefined_table THEN NULL;
END $$;
DO $$ BEGIN DROP POLICY IF EXISTS "Org Expenses Access" ON expenses;
EXCEPTION
WHEN undefined_table THEN NULL;
END $$;
DO $$ BEGIN DROP POLICY IF EXISTS "RBAC Objectives Select" ON objectives;
EXCEPTION
WHEN undefined_table THEN NULL;
END $$;
DO $$ BEGIN DROP POLICY IF EXISTS "Org Objectives Access" ON objectives;
EXCEPTION
WHEN undefined_table THEN NULL;
END $$;
DO $$ BEGIN DROP POLICY IF EXISTS "Users can manage own notifications" ON notifications;
EXCEPTION
WHEN undefined_table THEN NULL;
END $$;
DO $$ BEGIN DROP POLICY IF EXISTS "Org Notifications Access" ON notifications;
EXCEPTION
WHEN undefined_table THEN NULL;
END $$;
-- =====================================================
...
SELECT USING (id::text = get_my_organization_id()::text);
CREATE POLICY "Org admins can update organization" ON organizations FOR
UPDATE USING (
        id::text = get_my_organization_id()::text
        AND is_org_admin()
    );
-- PROFILES
CREATE POLICY "Profiles org isolation" ON profiles FOR
SELECT USING (
        id::text = auth.uid()::text
        OR organization_id::text = get_my_organization_id()::text
    );
-- CONTACTS
CREATE POLICY "Org Contact Access" ON contacts FOR ALL USING (
    organization_id::text = get_my_organization_id()::text
    AND (
        get_my_role() IN ('master', 'admin', 'manager')
        OR (
            get_my_role() = 'supervisor'
            AND (
                zone_id::text = get_my_zone_id()::text
                OR zone_id IS NULL
            )
        )
        OR (
...
-- DOCTORS
CREATE POLICY "Org Doctors Access" ON doctors FOR ALL USING (
    organization_id::text = get_my_organization_id()::text
    AND (
        get_my_role() IN ('master', 'admin', 'manager')
...
-- PHARMACIES
CREATE POLICY "Org Pharmacies Access" ON pharmacies FOR ALL USING (
    organization_id::text = get_my_organization_id()::text
    AND (
        get_my_role() IN ('master', 'admin', 'manager')
        OR get_my_role() = 'supervisor'
...
-- HEALTH CENTERS
CREATE POLICY "Org Health Centers Access" ON health_centers FOR ALL USING (
    organization_id::text = get_my_organization_id()::text
    AND (
        get_my_role() IN ('master', 'admin', 'manager')
        OR (
            get_my_role() = 'supervisor'
            AND zone_id::text = get_my_zone_id()::text
        )
        OR (
            get_my_role() = 'representative'
            AND user_id::text = auth.uid()::text
...
-- PRODUCTS
CREATE POLICY "Org Products Access" ON products FOR ALL USING (
    organization_id::text = get_my_organization_id()::text
) WITH CHECK (
    organization_id::text = get_my_organization_id()::text
...
-- VISITS
CREATE POLICY "Org Visits Access" ON visits FOR ALL USING (
    organization_id::text = get_my_organization_id()::text
    AND (
        get_my_role() IN ('master', 'admin', 'manager')
        OR (
            get_my_role() IN ('supervisor', 'telemarketing')
            AND zone_id::text = get_my_zone_id()::text
        )
        OR (
            get_my_role() = 'representative'
...
-- ZONES
CREATE POLICY "Org Zones Access" ON zones FOR ALL USING (
    organization_id::text = get_my_organization_id()::text
    AND get_my_role() IN ('master', 'admin', 'manager', 'supervisor')
) WITH CHECK (
    organization_id::text = get_my_organization_id()::text
    AND get_my_role() IN ('master', 'admin')
);
-- USER ROLES
CREATE POLICY "Org User Roles Access" ON user_roles FOR ALL USING (
    organization_id::text = get_my_organization_id()::text
    AND (
        get_my_role() = 'master'
...
-- TABLAS OPCIONALES
DO $$ BEGIN CREATE POLICY "Org Transfer Orders Access" ON transfer_orders FOR ALL USING (
    organization_id::text = get_my_organization_id()::text
    AND (
        get_my_role() IN ('master', 'admin', 'manager')
        OR (
...
WHEN undefined_table THEN NULL;
END $$;
DO $$ BEGIN CREATE POLICY "Org Expenses Access" ON expenses FOR ALL USING (
    organization_id::text = get_my_organization_id()::text
    AND (
        get_my_role() IN ('master', 'admin', 'manager')
        OR (
...
WHEN undefined_table THEN NULL;
END $$;
DO $$ BEGIN CREATE POLICY "Org Objectives Access" ON objectives FOR ALL USING (
    organization_id::text = get_my_organization_id()::text
    AND (
        get_my_role() IN ('master', 'admin', 'manager')
        OR (
...
WHEN undefined_table THEN NULL;
END $$;
DO $$ BEGIN CREATE POLICY "Org Notifications Access" ON notifications FOR ALL USING (
    organization_id::text = get_my_organization_id()::text
    AND user_id::text = auth.uid()::text
) WITH CHECK (
    organization_id::text = get_my_organization_id()::text
