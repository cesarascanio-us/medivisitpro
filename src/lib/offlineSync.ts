...
export interface PendingOperation {
    ...
    batchId?: string;
    timestamp: number;
    type: 'visit' | 'order' | 'sample_movement' | 'expense' | 'batch_visit';
...
export async function enqueueBatchVisitSync(
    visitPayload: {
        visit: Record<string, unknown>;
        sampleDrops?: Array<Record<string, unknown>>;
    ...
    const op: PendingOperation = {
        ...
        action: 'batch',
        table: 'visits',
        data: visitPayload as unknown as Record<string, unknown>,
        retryCount: 0,
        version: 1
    };
    await db.put('pendingOperations', op);
    console.log('[OfflineSync] Atomic Batch Visit enqueued:', batchId);
    return batchId;
}
