...
export default function Doctors() {
    ...
    const loadDoctors = async () => {
        try {
            ...
            let query: any = supabase.from('doctors').select('*, specialties(name)');
    ...
    const handleSubmit = async () => {
        ...
        try {
            const dataToSave = {
                ...
                state: formData.state || null, health_center: formData.health_center || null, health_center_id: formData.health_center_id || null, days: formData.days || null,
                ...
                instagram: formData.instagram || null, notes: formData.notes || null, priority: formData.priority || 'medium',
            };
    ...
    const handleImport = async (data: Record<string, any>[]) => {
        try {
            // Step 1: Get all profiles to map emails/usernames to UUIDs
            const { data: profiles } = await supabase
                .from('profiles')
                .select('user_id, email, first_name, last_name');

            const profileByEmail = new Map(
                (profiles || []).map(p => [p.email?.toLowerCase(), p.user_id])
            );

            // Step 2: Get all specialties to map IDs to names
            const { data: specialties } = await supabase
                .from('specialties')
                .select('id, name');

            const specialtyById = new Map(
                (specialties || []).map(s => [String(s.id), s.name])
            );

            // Helper: Convert Spanish time "9:00 a. m." / "2:00 p. m." → "09:00:00" / "14:00:00"
            const parseSpanishTime = (timeStr: any): string | null => {
                if (!timeStr) return null;
                const str = String(timeStr).trim().toLowerCase()
                    .replace(/\./g, '').replace(/\s+/g, ' ').trim(); // "9:00 a m" or "2:00 p m"
                const match = str.match(/(\d{1,2}):(\d{2})\s*([ap])/);
            ...
            // Legacy mappings from CSV
            const LEGACY_SPECIALTIES: Record<string, string> = {
                "1": "Pediatría", "2": "Medicina General", "3": "Gastroenterólogo",
                "4": "Urología", "5": "Nefrólogo", "6": "Medicina Interna",
                "7": "Ginecólogo", "8": "Cardiólogo", "9": "Traumatólogo", "10": "Oftalmólogo"
            };

            const LEGACY_HEALTH_CENTERS: Record<string, string> = {
                "1": "Especialidades Médicas Turmero", "2": "Centro Médico Maracay",
                "3": "Centro Clínico La Fontana", "4": "CEMAT", "5": "Maternidad La Floresta",
                "6": "Instituto Policlínico de Turmero", "7": "Centro Quirúrgico San José",
                "8": "Servicio Médico Star Medical", "9": "Centro de Especialidades Médicas",
                "10": "Clínica Calicanto", "11": "Clínica Aragua La Victoria", "12": "CREHAN",
                "13": "ASODIAM", "14": "Clínica Lugo", "15": "Policlínica Coromoto",
                "16": "UNIMED", "17": "Centro Médico San José", "18": "UNIGASTRO"
            };

            // Step 3: Map CSV rows to doctor records
            const doctorsToInsert = data.map((row: any) => {
                // Map representative email to UUID

                const repEmail = (row['ID_RepresentanteAsignado'] || row['representante'] || '').toString().toLowerCase();
                const representativeUUID = profileByEmail.get(repEmail) || user?.id;

                // Map specialty ID to name
                const specialtyRaw = String(row['Especialidad'] || row['especialidad'] || '').trim();
                const specialtyName = specialtyById.get(specialtyRaw) || LEGACY_SPECIALTIES[specialtyRaw] || specialtyRaw;

                // Encontrar el UUID real de la especialidad en la base de datos
                const dbSpecialty = specialties?.find(s => 
                    s.name.toLowerCase() === specialtyName.toLowerCase() ||
                    s.name.toLowerCase().includes(specialtyName.toLowerCase().replace('ólogo', '').replace('ó', 'o'))
                );
                const resolvedSpecialtyId = dbSpecialty ? dbSpecialty.id : null;

                // State comes as code (ARA, CAR, etc.) — store as-is, view joins to name
                const stateCode = (row['Estado'] || row['estado'] || '').toString().trim();

                // City comes as code (MCY, VAL, etc.) — store as-is
                const cityCode = (row['Ciudad'] || row['ciudad'] || '').toString().trim();

                // Health center is a numeric ID in the CSV
                const healthCenterRaw = String(row['Centro de Salud'] || row['Centro'] || row['centro'] || '').trim();
                const healthCenterName = LEGACY_HEALTH_CENTERS[healthCenterRaw] || healthCenterRaw;

                return {
                    user_id: representativeUUID || user?.id,
                    organization_id: organizationId,
                    representative_id: representativeUUID || user?.id,
                    ...
                    mobile: row['Movil'] || row['movil'] || null,
                    email: row['email'] || row['Email'] || null,
                    address: row['Dirección'] || row['Direccion'] || row['direccion'] || null,
                    location: row['Ubicación'] || row['Ubicacion'] || null,
                    days: row['Dias'] || row['Días'] || null,
                    start_time: parseSpanishTime(row['Hora Inicio']),
                    end_time: parseSpanishTime(row['Hora Fin']),
                    potential: row['Potencial'] || 'Medio',
                    observations: row['Observación'] || row['Observacion'] || null,
                    msds: row['MSDS'] ? String(row['MSDS']) : null,
                    cm: row['CM'] ? String(row['CM']) : null,
            ...
            if (doctorsToInsert.length > 0) {
                ...
                toast({ 
                    ...
                    description: `Se importaron ${doctorsToInsert.length} médicos con sus representantes asignados.` 
                });
                loadDoctors();
            }
        } catch (error: any) { 
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <Button variant="outline" onClick={async () => {
                            try {
                                setLoading(true);
                                await loadDoctors();
                                toast({ title: "Sincronización Completada", description: "Datos de especialistas actualizados." });
                            } catch (e: any) {
