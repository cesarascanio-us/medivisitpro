  ...
  const loadVisits = async () => {
    ...
    try {
      ...
      if (visitsData && visitsData.length > 0) {
        const contactIds = Array.from(new Set(visitsData.map(v => v.contact_id || v.pharmacy_id).filter(Boolean)));
        if (contactIds.length > 0) {
          const { data: contactsData, error: contactsError } = await supabase.from('unified_contacts').select('*').in('id', contactIds);
          if (!contactsError && contactsData) {
            const enrichedVisits = visitsData.map(visit => ({ ...visit, unified_contacts: contactsData.find(c => c.id === (visit.contact_id || visit.pharmacy_id)) || null }));
