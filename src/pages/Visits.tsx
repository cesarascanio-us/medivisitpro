...
export default function Visits() {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      loadVisits();
    } catch (error) { toast({ title: "Fallo de Sistema", description: "No se pudo eliminar el registro de visita.", variant: "destructive" }); }
  }
  ...
  return (
    <div className="flex flex-col h-full space-y-10 font-display animate-in fade-in duration-700 text-foreground pb-20">
      <EliteHeader 
        ...
        rightContent={
          <div className="flex items-center gap-4">
            <EliteButton variant="secondary" onClick={() => setShowHelp(!showHelp)} className={cn("h-14 w-14 p-0", showHelp && "bg-primary/10 text-primary border-primary/20")} icon={Lightbulb} />
            <EliteButton onClick={() => setWizardOpen(true)} icon={Plus}>Nueva Visita</EliteButton>
          </div>
        }
      />
      ...
      <div className="flex flex-wrap items-center gap-4 bg-muted/10 backdrop-blur-md p-6 rounded-[2.5rem] border border-border/40 shadow-inner">
        <EliteButton variant="ghost" onClick={handlePrint} icon={Printer} className="h-12 border border-border/40 bg-card">Imprimir Reporte</EliteButton>
        <div className="flex items-center h-12 shadow-sm rounded-xl overflow-hidden border border-border/40 bg-card">
          <EliteButton variant="ghost" className="h-full px-8 border-none" onClick={() => document.getElementById('import-input')?.click()} disabled={importing} icon={importing ? RefreshCw : Upload}>Importar Ciclo</EliteButton>
          <input type="file" id="import-input" className="hidden" onChange={handleImport} accept=".xlsx,.csv" />
          <EliteButton variant="ghost" size="icon" onClick={() => setHelpDialogOpen(true)} className="h-full w-12 border-l border-border/40 rounded-none" icon={HelpCircle} />
        </div>
        <EliteButton variant="ghost" onClick={() => exportToCSV(filteredVisits, 'visitas')} icon={Download} className="h-12 border border-border/40 bg-card">Exportar Inteligencia</EliteButton>
      </div>
      ...
      <Tabs defaultValue="list" className="w-full space-y-10">
        ...
        <TabsContent value="list" className="space-y-8 animate-in slide-in-from-bottom-4 duration-700">
          <EliteCard className="p-8">
            <div className="flex flex-col md:flex-row gap-6">
              <div className="flex-1">
                <EliteInput icon={Search} placeholder="FILTRAR POR MÉDICO, ESPECIALIDAD O ACTIVIDAD..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
              </div>
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                <SelectTrigger className="h-14 w-full md:w-72 bg-muted/5 border-border/40 rounded-elite-md font-black uppercase text-[10px] tracking-widest shadow-inner px-8">
                  <div className="flex items-center gap-3"><Filter className="h-4 w-4 text-primary" /><SelectValue placeholder="ESTADO OPERATIVO" /></div>
                </SelectTrigger>
          ...
          <div className="grid grid-cols-1 gap-8">
            {filteredVisits.map((visit, i) => (
              <EliteCard key={visit.id} delay={i * 50} className="group overflow-hidden">
                  ...
                  <div className="flex flex-col lg:flex-row gap-10">
                    <div className="flex-1 space-y-8">
                      <div className="flex flex-wrap items-center justify-between gap-6">
                        <div className="flex items-center gap-6">
                          <div className="w-16 h-16 rounded-2xl bg-muted/20 flex items-center justify-center border border-border/40 group-hover:bg-primary/10 group-hover:border-primary/30 transition-all duration-500 shadow-inner"><User className="h-7 w-7 text-primary" /></div>
                          <div className="flex flex-col gap-1">
                            <h3 className="text-xl font-black text-foreground uppercase tracking-tighter group-hover:text-primary transition-colors font-display">{visit.unified_contacts?.name || "CONTACTO NO IDENTIFICADO"}</h3>
                            <div className="flex items-center gap-4 mt-1">
                              <Badge variant="outline" className="badge-elite-info">{visit.unified_contacts?.specialty || "SIN ESPECIALIDAD"}</Badge>
                              {visit.unified_contacts?.potential === 'Alto' && <Badge className="badge-elite-success animate-pulse">PARETO ELITE</Badge>}
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-muted/5 p-8 rounded-[2rem] border border-border/40 shadow-inner">
                        <div className="space-y-4">
                          <div className="flex items-center text-elite-xs text-muted-foreground"><MapPin className="h-4 w-4 mr-4 text-primary" /><span className="truncate">{visit.unified_contacts?.address || "UBICACIÓN PROTEGIDA"}</span></div>
                          <div className="flex items-center text-elite-xs text-muted-foreground"><Clock className="h-4 w-4 mr-4 text-primary" />{new Date(visit.scheduled_date).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })} HORAS</div>
                        </div>
                        <div className="space-y-4">
                          <div className="flex items-center text-elite-xs text-muted-foreground"><Calendar className="h-4 w-4 mr-4 text-primary" />{new Date(visit.scheduled_date).toLocaleDateString('es-ES', { day: '2-digit', month: 'long', year: 'numeric' }).toUpperCase()}</div>
                          <div className="flex items-center text-elite-xs text-primary font-black uppercase tracking-widest"><FileText className="h-4 w-4 mr-4" /> ID: {visit.id.substring(0, 8).toUpperCase()}</div>
                        </div>
                      </div>
                      {visit.objective && (
                    ...
                    <div className="flex lg:flex-col justify-end gap-4 min-w-[240px]">
                      {visit.status === 'completed' && (
                        <VisitReportDialog trigger={<EliteButton variant="ghost" className="w-full h-14 border border-border/40 bg-card">Analizar Reporte</EliteButton>} visitData={visit} />
                      )}
                      {visit.status === 'scheduled' && (
                        <EliteButton className="w-full h-16" onClick={() => navigate(`/visits/execution/${visit.id}`)} icon={ArrowRight}>Iniciar Visita</EliteButton>
                      )}
                      <div className="flex gap-3">
                         <EliteButton variant="ghost" className="flex-1 h-14 border border-border/40 bg-card" icon={Edit}>Editar</EliteButton>
                         <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button variant="ghost" className="h-14 w-14 rounded-2xl text-muted-foreground hover:text-rose-500 hover:bg-rose-500/10 border border-border/40 shadow-inner"><Trash2 className="h-5 w-5" /></Button>
                          </AlertDialogTrigger>
        ...
        <TabsContent value="calendar" className="animate-in slide-in-from-bottom-4 duration-700">
          <EliteCard className="p-12">
            <div className="flex flex-col xl:flex-row gap-16">
              <div className="flex-1">
                <div className="bg-muted/5 p-10 rounded-[3rem] border border-border/40 shadow-inner">
                  <div className="flex items-center justify-between mb-10">
                    <h3 className="text-2xl font-black text-foreground uppercase tracking-tighter font-display">Vista de Calendario</h3>
                    <Badge className="badge-elite-info px-4 py-1.5 rounded-full">Visión del Ciclo</Badge>
                  </div>
              ...
              <div className="w-full xl:w-[450px] space-y-12">
                <div>
                  <h3 className="text-xl font-black text-foreground uppercase tracking-tighter mb-8 ml-2 font-display">Consolidado de Actividades</h3>
                ...
                <div className="pt-10 border-t border-border/40">
                  ...
                  <div className="space-y-4">
                    {visits.filter(v => new Date(v.scheduled_date) >= new Date() && v.status === 'scheduled').sort((a, b) => new Date(a.scheduled_date).getTime() - new Date(b.scheduled_date).getTime()).slice(0, 5).map(v => (
                        <div key={v.id} className="flex items-center justify-between p-6 bg-muted/5 hover:bg-card hover:shadow-premium-sm border border-transparent hover:border-border/40 rounded-[1.5rem] transition-all group cursor-pointer" onClick={() => navigate(`/visits/execution/${v.id}`)}>
                          <div className="flex items-center gap-5">
                             ...
                             <div className="flex flex-col gap-1">
                               ...
                               <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest opacity-60">{new Date(v.scheduled_date).toLocaleDateString('es-ES', { day: '2-digit', month: 'short' }).toUpperCase()} • {new Date(v.scheduled_date).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}</p>
                             </div>
                          </div>
                          <EliteButton variant="ghost" size="icon" className="h-10 w-10 p-0" icon={ArrowRight} />
                        </div>
                    ))}
      ...
      <Dialog open={helpDialogOpen} onOpenChange={setHelpDialogOpen}>
        <DialogContent className="max-w-[750px] bg-card rounded-[3.5rem] border border-border/40 shadow-premium-2xl p-0 overflow-hidden font-display">
          <div className="bg-muted/5 p-12 border-b border-border/40">
            <div className="flex items-center gap-6">
              <div className="w-16 h-16 rounded-3xl bg-card flex items-center justify-center shadow-premium-sm border border-border/40"><FileSpreadsheet className="h-8 w-8 text-primary" /></div>
              <div>
          ...
          <div className="p-12 space-y-10">
            <div className="bg-primary/5 p-8 rounded-[2rem] border border-primary/10 flex gap-6">
            ...
            <div className="flex justify-end pt-4"><EliteButton onClick={() => setHelpDialogOpen(false)}>Entendido</EliteButton></div>
          </div>
        </DialogContent>
      </Dialog>
    </div>
