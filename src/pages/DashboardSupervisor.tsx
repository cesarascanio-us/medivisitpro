...
export default function DashboardSupervisor() {
    ...
    const loadDashboardData = async () => {
        setLoading(true);
        try {
            // Get representatives assigned to this supervisor either explicitly or by geography
            const { data: roleData } = await supabase
                .from('user_roles')
                .select('user_id, supervisor_id, region, state')
                .in('role', ['representative', 'rep_comercial', 'visitador_medico', 'rep_integral']);

            const repIdsForSupervisor = (roleData || [])
                .filter(r => r.supervisor_id === user?.id || (!r.supervisor_id && (r.region === userRegion || r.state === userState)))
                .map(r => r.user_id);
