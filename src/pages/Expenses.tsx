...
export default function Expenses() {
    const { user, canViewAllData, isSupervisor, isCoordinator, isManager, zoneId, organizationName } = useAuth();
    ...
    const updateExpenseStatus = async (expenseId: string, action: 'approve' | 'reject') => {
        try {
            let updates: any = {};
            if (action === 'reject') {
                updates = { status: 'rejected', workflow_status: 'rejected' };
            } else {
                // Determine escalation based on role
                if (isSupervisor) {
                    updates = { workflow_status: 'pending_coordinator', approved_by_supervisor_id: user?.id };
                } else if (isCoordinator) {
                    updates = { workflow_status: 'pending_manager', approved_by_coordinator_id: user?.id };
                } else if (isManager) {
                    updates = { workflow_status: 'pending_finance', approved_by_manager_id: user?.id, status: 'approved' };
                } else {
                    updates = { status: 'approved' }; // Fallback
                }
            }
            ...
            toast({
                title: action === 'approve' ? "Validación & Escalamiento Positivo" : "Registro Rechazado",
                description: `El estatus del reporte ha ascendido en la cadena de mando.`
            });
            loadExpenses();
        } catch (error) {
            toast({
    ...
    const getStatusBadge = (status: string, workflow_status?: string) => {
        const wf = workflow_status || status;
        const styles: Record<string, string> = {
            pending: "bg-amber-500/10 text-amber-500 border-amber-500/20",
            pending_supervisor: "bg-amber-500/10 text-amber-500 border-amber-500/20",
            pending_coordinator: "bg-blue-500/10 text-blue-500 border-blue-500/20",
            pending_manager: "bg-indigo-500/10 text-indigo-500 border-indigo-500/20",
            pending_finance: "bg-fuchsia-500/10 text-fuchsia-500 border-fuchsia-500/20",
        ...
        const labels: Record<string, string> = {
            ...
            pending_finance: "EN CAJA / PAGOS",
            approved: "VALIDADO",
            rejected: "RECHAZADO",
            paid: "LIQUIDADO"
        };
        return <Badge className={cn("font-black text-[9px] uppercase tracking-widest px-3 py-1 border", styles[wf] || "bg-muted")}>{labels[wf] || wf}</Badge>;
    };
                ...
                <Table>
                    <TableHeader>
                        <TableRow className="hover:bg-transparent border-border/40">
                            ...
                            <TableHead className="text-center text-[10px] font-black uppercase tracking-widest py-6">Estatus Workflow</TableHead>
                            {(isManager || isCoordinator || isSupervisor) && <TableHead className="text-right text-[10px] font-black uppercase tracking-widest py-6 pr-8">Control</TableHead>}
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {loading ? (
                                ...
                                return (
                                    <TableRow key={expense.id} className="hover:bg-muted/5 transition-colors border-border/40 group">
                                        ...
                                        <TableCell className="text-center">
                                            {getStatusBadge(expense.status, expense.workflow_status)}
                                        </TableCell>
                                        {(isManager || isCoordinator || isSupervisor) && (
                                            <TableCell className="text-right pr-8 py-8">
                                                {/* Logic for who can approve based on workflow_status */}
                                                {((isSupervisor && (expense.workflow_status === 'pending_supervisor' || !expense.workflow_status && expense.status === 'pending')) ||
                                                  (isCoordinator && expense.workflow_status === 'pending_coordinator') ||
                                                  (isManager && expense.workflow_status === 'pending_manager')) && (
                                                    <div className="flex justify-end gap-3">
                                                        <Button
                                                            ...
                                                            onClick={() => updateExpenseStatus(expense.id, 'approve')}
                                                            title="Aprobar y Escalar"
                                                        >
                                                        ...
                                                        <Button
                                                            ...
                                                            onClick={() => updateExpenseStatus(expense.id, 'reject')}
                                                            title="Rechazar"
                                                        >
