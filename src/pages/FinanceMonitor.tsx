...
export default function FinanceMonitor() {
    ...
    const loadFinanceData = async () => {
        try {
            ...
            if (zoneIds.length > 0) {
                // Query the roles table instead of profiles
                const { data: userData } = await supabase.from('user_roles').select('user_id').in('zone_id', zoneIds);
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <Card className="border border-border/40 shadow-premium-lg rounded-lg overflow-hidden bg-card text-card-foreground relative">
                    ...
                    <CardContent className="p-8 space-y-8 relative z-10">
                        <div className="text-center py-4">
                            <h2 className="text-5xl font-bold mb-2 tracking-tighter text-foreground">{stats.netEfficiency.toFixed(1)}%</h2>
                            <p className="text-muted-foreground font-bold uppercase text-xs tracking-wider">Ratio de gastos</p>
                        </div>
                        
                        <div className="space-y-5">
                            <div className="bg-muted/50 p-5 rounded-lg border border-border/50">
                                <div className="flex justify-between items-center mb-2">
                                    <span className="text-xs font-bold text-foreground">Salud financiera</span>
                            ...
                            <Button variant="secondary" size="default" className="w-full mt-2 border border-border/50">
