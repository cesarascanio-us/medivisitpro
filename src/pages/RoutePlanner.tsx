...
const CONTACT_TYPES = [
    { id: 'doctor',       icon: UserRound,   label: 'Médico', types: ['doctor'] },
    { id: 'hospital',     icon: Building2,   label: 'Centro', types: ['health_center', 'hospital', 'clinic'] },
    { id: 'pharmacy',     icon: Store,        label: 'Farmacia', types: ['pharmacy'] },
    { id: 'natural_store',icon: LeafIcon,     label: 'Naturista', types: ['natural_store'] },
    { id: 'commerce',     icon: ShoppingCart, label: 'Comercio', types: ['commerce'] },
    { id: 'drugstore',    icon: Truck,        label: 'Droguería', types: ['drugstore'] },
];
...
export default function RoutePlanner() {
    ...
    const getFilteredContacts = () =>
        contacts.filter(c => {
            const typeConfig = CONTACT_TYPES.find(t => t.id === selectedType);
            const matchesType = typeConfig ? typeConfig.types.includes(c.contact_type) : c.contact_type === selectedType;
            return matchesType && (c.name || '').toLowerCase().includes((search || '').toLowerCase());
        });
    ...
    const loadContacts = async () => {
        try {
            ...
            if (hcData) {
                const hcFormatted = hcData.map(hc => ({
                    ...hc,
                    specialty: null,
                    contact_type: "health_center",
                    source: "health_centers"
                }));
                allData = [...allData, ...hcFormatted];
            }

            if (allData.length === 0) return;

            // Load specialties to map IDs to names
            const { data: specs, error: specsError } = await supabase.from('specialties').select('id, name');
            ...
            const sm: Record<string, string> = { ...LEGACY_SPECIALTIES };
            if (specs) {
                specs.forEach(s => sm[String(s.id)] = s.name);
            }
            setSpecialtiesMap(sm);

            // Enrich with routing days from source tables
            const enriched = await Promise.all(
                allData.map(async (contact) => {
                    try {
                        ...
                        <div className="grid grid-cols-3 lg:grid-cols-6 gap-3">
                            {CONTACT_TYPES.map((type) => {
                                const countInType = contacts.filter(c =>
                                    type.types.includes(c.contact_type) &&
                                    c.routing_days?.split(",").map((d: string) => d.trim()).includes(selectedDay)
                                ).length;
                                const totalInType = contacts.filter(c => type.types.includes(c.contact_type)).length;
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    {(contact.specialty || contact.address) && (
                                                        <p className="text-[10px] text-muted-foreground truncate">
                                                            {specialtiesMap[contact.specialty] || contact.specialty || contact.address}
                                                        </p>
                                                    )}
