...
import { Search, AlertTriangle, CheckCircle, Boxes, Tag, Calendar, History as HistoryIcon } from "lucide-react";
...
export default function InventoryKardex() {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-purple-50 p-4 rounded-xl border border-purple-100">
                <div className="flex items-center gap-3">
                    <Boxes className="h-6 w-6 text-purple-600" />
                    <h3 className="font-bold text-lg text-purple-900">Control de Existencias y Trazabilidad</h3>
                </div>
                <div className="relative w-full md:w-80">
                    ...
                    <Input
                        ...
                        className="pl-10 border-purple-200 focus-visible:ring-purple-500 bg-white"
                    />
                </div>
            </div>
            ...
            <Tabs defaultValue="consolidated" className="w-full">
                <TabsList className="mb-4 bg-purple-100 p-1">
                    <TabsTrigger value="consolidated" className="data-[state=active]:bg-white data-[state=active]:text-purple-700">
                        <Boxes className="mr-2 h-4 w-4" /> Stock Consolidado
                    </TabsTrigger>
                    <TabsTrigger value="batches" className="data-[state=active]:bg-white data-[state=active]:text-purple-700">
                        <Tag className="mr-2 h-4 w-4" /> Detalle por Lote
                    </TabsTrigger>
                    <TabsTrigger value="movements" className="data-[state=active]:bg-white data-[state=active]:text-purple-700">
                        <HistoryIcon className="mr-2 h-4 w-4" /> Movimientos
                    </TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="consolidated">
                    <div className="rounded-xl border shadow-sm bg-white overflow-hidden">
                        <Table>
                            <TableHeader className="bg-slate-50 text-slate-900 font-bold">
                                <TableRow>
                                ...
                                {filteredInventory.map((item, idx) => {
                                    ...
                                    return (
                                        <TableRow key={idx} className="hover:bg-slate-50/50">
                                            <TableCell className="font-medium text-purple-900">{item.product_name}</TableCell>
                                    ...
                                    <TableRow key={m.id} className="hover:bg-slate-50/50">
                                        ...
                                        <TableCell>
                                            <Badge variant="outline" className={`text-[10px] font-bold uppercase ${m.movement_type === 'inbound_purchase' ? 'text-green-600 border-green-200 bg-green-50' :
                                                m.movement_type === 'outbound_dispatch' ? 'text-purple-600 border-purple-200 bg-purple-50' :
