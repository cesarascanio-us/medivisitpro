...
export const CommissionCalculator = () => {
    ...
    const currentMonthlyIncome = visitsPerDay * 20 * (conversionRate / 100) * (avgCommission / 10); // Simplified logic
    // visits * 20 days * conversion rate * avg value per conversion
    // Let's make the math simpler for the demo:
    // Income = Visits * Days * Ticket * Conversion

    // Better math model:
    // Monthly Sales Volume = (Visits/Day * 20 Days) * (Conversion Rate%) * (Avg Ticket Value)
    // Commission = Monthly Sales Volume * Commission % (e.g. 5%)
    // Let's simulate "Potential Extra Earnings"
    ...
    return (
        <section className="py-20 px-4 sm:px-6 lg:px-8 bg-slate-800/30 border-y border-slate-800">
            <div className="max-w-4xl mx-auto">
                <div className="grid md:grid-cols-2 gap-12 items-center">
                        ...
                        <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-700 space-y-6">
                            <div className="space-y-3">
                                <div className="flex justify-between text-sm">
                                    <Label className="text-slate-300">Visitas diarias actuales</Label>
                                    <span className="text-white font-mono">{visitsPerDay}</span>
                                </div>
                                <Slider
                                    value={[visitsPerDay]}
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between text-sm">
                                    <Label className="text-slate-300">Comisión Mensual Promedio ($)</Label>
                                    <span className="text-white font-mono">${avgCommission}</span>
                                </div>
                                <Slider
                                    value={[avgCommission]}
                    ...
                    <div className="relative">
                        {showResult ? (
                            <div className="bg-gradient-to-br from-emerald-900/40 to-slate-900 border border-emerald-500/30 rounded-3xl p-8 text-center animate-in fade-in zoom-in duration-500">
                                <h3 className="text-slate-400 text-sm font-medium uppercase tracking-wider mb-2">Podrías ganar extra</h3>
                                <div className="text-5xl font-extrabold text-emerald-400 mb-2">
                                    +${extraEarnings}
                                    <span className="text-lg text-emerald-600 ml-1">/mes</span>
                                </div>
                                ...
                                <Button className="w-full mt-8 bg-white text-slate-900 hover:bg-slate-100 font-bold">
                                    Quiero esos ${extraEarnings} extra
                                </Button>
                            </div>
                        ) : (
                            <div className="flex flex-col items-center justify-center text-center h-full min-h-[300px] border-2 border-dashed border-slate-700 rounded-3xl p-8 opacity-50">
