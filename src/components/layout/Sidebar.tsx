...
export function Sidebar({ className, isMobile = false }: SidebarProps) {
  ...
  const getRoleGroup = () => {
    if (['master', 'admin', 'gerente', 'manager'].includes(role)) return 'master';
    if (['jefe', 'chief'].includes(role)) return 'ejecutivo';
    if (['coordinador', 'coordinator', 'supervisor', 'finance', 'operations'].includes(role)) return 'supervision';
    ...
    if (['farmacia', 'medico', 'compras', 'pharmacist', 'doctor', 'buyer', 'service_chief'].includes(role)) return 'externo';
  ...
  const NAV_GROUPS: Record<string, any[]> = {
    ejecutivo: [
      {
        ...
        visible: canSeeMaster || isAdmin,
        items: [
          { name: "Consola Sentinel", href: "/master-panel", icon: Crown, visible: canSeeMaster },
          { name: "Matriz de Roles", href: "/roles", icon: Shield, visible: canSeeMaster || isAdmin },
      ...
      },
      {
        ...
        items: [
          ...
          { name: "Administración Fin.", href: "/finance-admin", icon: DollarSign, visible: canSeeMaster || role === 'finance' },
          { name: "Búnker Operaciones", href: "/operations", icon: Package, visible: canSeeMaster || role === 'operations' },
        ]
      },
      {
        ...
        items: [
          ...
          { name: texts.coverage_title, href: "/coverage-map", icon: Map, visible: can('map.view') },
    ...
    supervision: [
      ...
      },
      {
        ...
        visible: true,
        items: [
          { name: "Muestras", href: "/samples", icon: Pill, visible: true },
  ...
  const masterGroups = [
    ...
    ...(NAV_GROUPS.supervision || []).map(g => ({
      ...g,
      items: g.items.filter(i => !['/dashboard', '/objectives', '/coverage-map'].includes(i.href))
    })),
    ...(NAV_GROUPS.campo || []).map(g => ({
      ...g,
      items: g.items.filter(i => !['/dashboard', '/objectives', '/route-planner', '/planner', '/visits', '/samples', '/material-pop', '/expenses', '/coverage-map', '/transfer-orders'].includes(i.href))
    }))
  ];

  let baseGroups = roleGroup === 'master' ? masterGroups : (NAV_GROUPS[roleGroup] || []);

  if (roleGroup === 'supervision') {
    // El supervisor debe ver lo que ve su representante, evitando duplicados
    const filteredCampo = (NAV_GROUPS['campo'] || []).map(g => ({
      ...g,
      items: g.items.filter(i => !['/dashboard', '/objectives', '/route-planner', '/expenses', '/samples', '/material-pop', '/transfer-orders', '/visits', '/coverage-map', '/planner'].includes(i.href))
    })).filter(g => g.items.length > 0);
    
    baseGroups = [...baseGroups, ...filteredCampo];
  }

  if (isAdmin && roleGroup !== 'ejecutivo' && roleGroup !== 'master') {
      baseGroups = [...(NAV_GROUPS['ejecutivo'] || []), ...baseGroups];
  }

  const developmentGroup = {
    title: "Desarrollo e Incentivos",
    visible: true,
    items: [
      { name: "Academia", href: "/university", icon: GraduationCap, visible: true },
      { name: "Premios", href: "/rewards", icon: Award, visible: true },
      { name: "Gestión de Academia", href: "/admin/academy", icon: SettingsIcon, visible: isMaster || isAdmin || isManager || role === 'gerente' }
    ]
  };
  ...
  return (
    <aside
      className={cn(
        "flex flex-col bg-card/95 backdrop-blur-[20px] border-r border-white/5 h-screen sticky top-0 flex-shrink-0 z-40 shadow-premium-lg",
    ...
    >
      ...
      {/* User Footer */}
      <div className={cn(
        "border-t border-white/5 p-2.5 flex-shrink-0",
        isExpanded ? "" : "flex justify-center"
      )}>
        <div className={cn(
          "flex items-center rounded-xl transition-colors",
          isExpanded ? "w-full justify-between gap-1 p-2" : "w-12 flex-col items-center gap-2 p-1"
        )}>
          {/* Avatar Area */}
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-9 h-9 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0 border border-primary/20 relative shadow-premium-sm">
              <span className="text-xs font-bold text-primary">{userInitials}</span>
              {!isExpanded && hasActiveVisit && (
                 <div className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-green-500 rounded-full shadow-[0_0_10px_rgba(34,197,94,0.5)] animate-pulse" />
              )}
            </div>
            ...
            {isExpanded && (
              <div className="min-w-0 flex-1 animate-in fade-in duration-200">
                <div className="flex items-center gap-1.5">
                  ...
                  {hasActiveVisit && (
                    <div className="w-1.5 h-1.5 rounded-full bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)] animate-pulse flex-shrink-0" title="Visita en curso" />
                  )}
                </div>
                <p className="text-[10px] text-muted-foreground truncate mt-0.5 tracking-wide uppercase font-medium">{getRoleLabel()}</p>
              </div>
            )}
          </div>

          <div className={cn("flex items-center", isExpanded ? "gap-1 flex-row" : "gap-2 flex-col")}>
            <ThemeToggle />
            <Button
              variant="ghost"
              size="icon"
              className={cn("flex-shrink-0 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg",
