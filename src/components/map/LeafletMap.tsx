...
// import MarkerClusterGroup from 'react-leaflet-cluster'; // Removed due to React 18 unmount bug
...
// --- HACK: Prevent React 18 startTransition unmount crashes ---
const originalRemove = L.Map.prototype.remove;
L.Map.prototype.remove = function() {
    try {
        originalRemove.call(this);
    } catch (e) {
        console.warn('Leaflet map remove caught error:', e);
    }
    return this;
};
...
// Also patch Layer removal to prevent Marker/Popup/Tooltip crash on unmount
const originalLayerRemove = L.Layer.prototype.remove;
L.Layer.prototype.remove = function() {
    try {
        originalLayerRemove.call(this);
    } catch (_e) {
        // Ignore unmount errors
    }
    return this;
...
const originalLayerRemoveFrom = L.Layer.prototype.removeFrom;
L.Layer.prototype.removeFrom = function(obj: any) {
    try {
        originalLayerRemoveFrom.call(this, obj);
    } catch (_e) {
        // Ignore unmount errors
    }
    return this;
...
interface LeafletMapProps {
    ...
    mapType?: 'roadmap' | 'satellite' | 'terrain' | 'hybrid' | 'light';
...
const getIconForType = (type: MapMarker['type'], customColor?: string, opacity: number = 1) => {
    ...
    switch (type) {
        ...
        case 'drugstore':
            ...
            break;
        case 'natural_store':
            color = customColor || '#84CC16'; // Lime
            emoji = '🌿';
            break;
        case 'commerce':
            color = customColor || '#F59E0B'; // Amber
            emoji = '🏪';
            break;
    }
                        ...
                        <p className="text-[10px] text-emerald-400 uppercase font-semibold leading-none mt-0.5">
                            {marker.type === 'doctor' ? 'Médico' :
                                marker.type === 'pharmacy' ? 'Farmacia' :
                                    marker.type === 'drugstore' ? 'Droguería' :
                                        marker.type === 'natural_store' ? 'Tienda Naturista' :
                                            marker.type === 'commerce' ? 'Comercio' :
                                                'Centro de Salud'}
                        </p>
                    </div>
                </Tooltip>
    ...
    return (
        <div style={{ height, width: '100%', borderRadius: '0.5rem', overflow: 'hidden', position: 'relative' }}>
            ...
            >
                ...
                {/* Standard Markers Layer (Clustering disabled to fix React 18 transition unmount crashes) */}
                <>
                    {markers.filter(m =>
                        m.position &&
                ...
                {showInfluenceCircles && markers.map((marker) => (
                    (marker.type === 'health_center' || marker.type === 'hospital' || marker.type === 'clinic') && (
                        <Circle
