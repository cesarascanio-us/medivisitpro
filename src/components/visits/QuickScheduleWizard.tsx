...
export function QuickScheduleWizard({ open, onOpenChange, onSuccess, visitData }: QuickScheduleWizardProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const { error } = await supabase.from('visits').insert([{ user_id: user.id, organization_id: organizationId, contact_id: contactId, scheduled_date: scheduledDateTime.toISOString(), visit_type: visitType, objective: fullObjective, products_presented: selectedProducts, promotional_materials: selectedMaterials.join(', '), status: 'scheduled' }]).select().single();
