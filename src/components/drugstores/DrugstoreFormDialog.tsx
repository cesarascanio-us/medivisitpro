...
import { Building2, User, Building, MapPin, ShieldCheck, Mail, Phone, Globe, Lock, Globe2 } from "lucide-react";
...
interface DrugstoreFormDialogProps {
    ...
    onSubmit: () => void;
    isEditing: boolean;
    canManageNacional?: boolean; // false = commercial_rep (solo puede crear Regionales)
}
                        ...
                        <TabsContent value="perfil" className="m-0 space-y-6 mt-0">
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    <Label className="text-[10px] font-black uppercase text-slate-500 ml-1">Tipo de Droguería</Label>
                                    {canManageNacional ? (
                                        <div className="flex gap-3">
                                            {(['Regional', 'Nacional'] as const).map((t) => (
                                                <button
                                                    key={t}
                                                    type="button"
                                                    onClick={() => setFormData({ ...formData, type: t })}
                                                    className={cn(
                                                        "flex-1 h-12 rounded-xl border-2 font-black text-[11px] uppercase tracking-widest transition-all",
                                                        (formData.type || 'Regional') === t
                                                            ? t === 'Nacional'
                                                ...
                                                >
                                                    {t === 'Nacional' ? <><Globe2 className="inline h-4 w-4 mr-2" />Nacional</> : <><MapPin className="inline h-4 w-4 mr-2" />Regional</>}
                                                </button>
                                            ))}
                                        </div>
                                    ) : (
                                        <div className="flex items-center gap-3 h-12 px-4 bg-muted/30 border border-border rounded-xl">
                                            <Lock className="h-4 w-4 text-muted-foreground" />
                                            <span className="font-black text-[11px] uppercase tracking-widest text-emerald-600">Regional</span>
                                            <span className="text-[10px] text-muted-foreground ml-auto">Asignado automáticamente</span>
                                        </div>
                                    )}
                                </div>
