...
export default function DashboardRep({ mode = 'comercial' }: DashboardRepProps) {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      if (!error && visitsData) {
        // Fetch contact details from unified_contacts
        const contactIds = Array.from(new Set(visitsData.map(v => v.contact_id).filter(Boolean)));
        let contactsData: any[] = [];
        if (contactIds.length > 0) {
           const { data } = await supabase.from('unified_contacts').select('*').in('id', contactIds);
           if (data) contactsData = data;
        }

        const mappedVisits: VisitItem[] = visitsData.map((v: any) => {
          const contact = contactsData.find(c => c.id === v.contact_id);
          const rawType = v.visit_type;
          const displayCategory = rawType === 'doctor' ? 'Médico' : 
                                  rawType === 'pharmacy' ? 'Farmacia' : 
                                  rawType === 'health_center' ? 'Centro de Salud' : 
                                  rawType === 'drugstore' ? 'Droguería' :
          ...
          return {
            ...
            address: contact?.address || "Sin dirección",
            status: v.status === "completed" ? "completed" : "pending",
            category: displayCategory
          };
        });
