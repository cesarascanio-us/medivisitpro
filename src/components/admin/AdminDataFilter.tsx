...
export function AdminDataFilter({ onFilterChange, moduleType = 'contacts' }: AdminDataFilterProps) {
    ...
    const visibleMembers = members.filter(m => {
        // 1. Restricción de rol: Supervisor solo ve a su equipo
        if (isStrictSupervisor && profile?.id) {
            const isDirectReport = m.supervisorId === profile.id;
            const isSameState = userState && m.state === userState;
            const isSameRegion = userRegion && m.region === userRegion;
            const isSubordinate = isDirectReport || (!m.supervisorId && (isSameState || isSameRegion));
            
            if (!(m.id === profile.id || isSubordinate)) {
                return false;
            }
        }
        
        // 2. Filtros activos de la UI
        if (filters.state && filters.state !== 'all') {
            if (m.state !== filters.state) return false;
        }
        if (filters.region && filters.region !== 'all') {
            if (m.region !== filters.region) return false;
        }
