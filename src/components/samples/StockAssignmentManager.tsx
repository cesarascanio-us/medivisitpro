...
export function StockAssignmentManager() {
    const { user, isMaster, canAssignStock, organizationId } = useAuth();
    ...
    // Check if user can assign to supervisors
    const canAssignToSupervisors = isMaster || canAssignStock;

    useEffect(() => {
        loadData();
    }, [demoData, organizationId, canAssignToSupervisors]);
