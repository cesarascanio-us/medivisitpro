...
export function useDashboardData(organizationId: string | undefined, zoneId?: string | undefined) {
  ...
  }, [organizationId, queryClient, isDemo]);
  ...
  const ciclo = useQuery({
    ...
    queryFn: async () => {
      if (isDemo) {
        return MOCK_DATA.cycles?.[0] || { name: 'Ciclo Vigente', status: 'active', start_date: new Date().toISOString(), end_date: new Date().toISOString() };
      }
  ...
  const representantes = useQuery({
    ...
    queryFn: async () => {
      if (isDemo) {
        return [
          { user_id: 'rep-1', role: 'representative', is_active: true, profiles: { first_name: 'Juan', last_name: 'Pérez' } },
          { user_id: 'rep-2', role: 'representative', is_active: true, profiles: { first_name: 'María', last_name: 'Gómez' } }
        ];
      }
      if (!organizationId) return [];
